...
bool DXGIGrabber::initCapture() {
    ...
    desc.Width            = m_config.sourceWidth;
    desc.Height           = m_config.sourceHeight;
    desc.MipLevels        = 1;
    desc.ArraySize        = 1;
...
bool DXGIGrabber::captureFrame(FrameProcessor& processor) {
    Microsoft::WRL::ComPtr<IDXGIResource>        resource;
    DXGI_OUTDUPL_FRAME_INFO                      frameInfo{};

    HRESULT hr = m_duplication->AcquireNextFrame(100, &frameInfo, &resource);
    if (hr == DXGI_ERROR_WAIT_TIMEOUT) return true;  // no new frame, skip
    if (FAILED(hr)) return false;
